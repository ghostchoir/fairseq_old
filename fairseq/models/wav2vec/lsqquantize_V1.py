...
batch of activations, respectively
        ...
        if self.init_state == 0:
            ...
            self.s.data = 0.9 * self.s.data + 0.1 ** torch.mean(torch.abs(activation.detach())) * 2 / (
                math.sqrt(self.Qp))
            self.init_state += 1
        elif self.init_state == self.batch_init:
...
class LSQWeightQuantizer(nn.Module):
    ...
    def forward(self, weight):
        if self.init_state == 0:
            ...
            if self.per_channel:
                weight_tmp = weight.detach().contiguous().view(weight.size()[0], -1)
                self.s.data = 0.9 * self.s.data + 0.1 * torch.mean(torch.abs(weight_tmp), dim=1) * 2 / (
                    math.sqrt(self.Qp))
            else:
                self.s.data = 0.9 * self.s.data + 0.1 * torch.mean(torch.abs(weight.detach())) * 2 / (
                    math.sqrt(self.Qp))
            self.init_state += 1
        elif self.init_state == self.batch_init:
            # self.s = torch.nn.Parameter(self.s)
...
class QuantConv2d(nn.Conv2d):
    def __init__(self,
        ...
        self.activation_quantizer = LSQActivationQuantizer(a_bits=a_bits, all_positive=all_positive,
                                                           batch_init=batch_init)
        self.weight_quantizer = LSQWeightQuantizer(w_bits=w_bits, all_positive=all_positive, per_channel=per_channel,
                                                   batch_init=batch_init)
