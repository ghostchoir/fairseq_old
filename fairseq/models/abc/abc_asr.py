...
def add_common_args(parser):
    parser.add_argument("--abc-path", help="path to abc model")
    parser.add_argument(
...
@register_model("abc_ctc")
class ABCCtc(BaseFairseqModel):
    @staticmethod
    def add_args(parser):
        """Add model-specific arguments to the parser."""
        add_common_args(parser)

    def __init__(self, abc_encoder, args):
        super().__init__()
        self.abc_encoder = abc_encoder
        self.args = args
    ...
    def build_model(cls, args, task):
        ...
        abc_encoder = ABCEncoder(args, task.target_dictionary)
        return cls(abc_encoder, args)
...
class ABCEncoder(FairseqEncoder):
    def __init__(self, args, tgt_dict=None):
        ...
        if getattr(args, "abc_args", None) is None:
            state = checkpoint_utils.load_checkpoint_to_cpu(
                args.abc_path, arg_overrides
            )
            abc_args = state["args"]
        else:
            state = None
            abc_args = args.abc_args

        assert args.normalize == abc_args.normalize, 'Fine-tuning works best when data normalization is the same'

        abc_args.data = args.data
        task = tasks.setup_task(abc_args)
        model = task.build_model(abc_args)
        ...
        d = abc_args.encoder_embed_dim

        self.abc_model = model
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            x, padding_mask = self.abc_model.extract_features(**abc_args)
...
@register_model_architecture("abc_ctc", "abc_ctc")
...
@register_model_architecture("abc_seq2seq", "abc_seq2seq")
